...
export function readUVarInt(buffer: Uint8Array): number {
  let value = Number(0);
  let shifter = Number(0);
  let index = 0;
...
export function readVarInt(buffer: Uint8Array): number {
...
export function uVarIntNumber(buffer: number[]): [number, number[]] {
